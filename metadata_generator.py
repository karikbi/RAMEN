...
class CategoryClassifier:
    ...
    def _precompute_category_embeddings(self):
        ...
        try:
            ...
            text_inputs = self.processor(
                text=category_texts, padding=True, return_tensors="pt"
            )
    ...
    def classify(
        self, image_embedding: np.ndarray, top_k: int = 2, threshold: float = 0.25
        ...
        try:
            ...
            with torch.no_grad():
                similarities = torch.matmul(
                    image_tensor, self.category_text_features.T
                )[0]
...
class MetadataGenerator:
    ...
    def extract_color_palette(
        self, bgr: np.ndarray, n_colors: int = 5
    ...
    def detect_composition_type(self, gray: np.ndarray) -> tuple[str, float, float]:
        ...
        if left.shape == right.shape:
            symmetry = (
                1.0 - np.mean(np.abs(left.astype(float) - right.astype(float))) / 255
            )
        else:
            symmetry = 0.5
        ...
        thirds_score = sum(i.sum() for i in intersections if i.size > 0) / (
            255 * 40 * 40 * 4 + 1
        )

        # Detect leading lines
        lines = cv2.HoughLinesP(
            edges, 1, np.pi / 180, 100, minLineLength=min(h, w) // 3, maxLineGap=10
        )
    ...
    def infer_category_from_metadata(
        self, title: str, source_metadata: dict
    ) -> tuple[str, list[str]]:
    ...
    def generate_metadata(
        ...
        try:
            ...
            if siglip_embedding is not None and self.category_classifier is not None:
                ...
                ml_categories = self.category_classifier.classify(
                    siglip_embedding, top_k=2, threshold=0.25
                )
                ...
                if ml_categories:
                    ...
                    logger.info(
                        f"ML category classification: {primary_cat} (confidence: {confidence:.2f})"
                    )
                else:
                    # Fallback to keyword matching if ML fails
                    primary_cat, subcats = self.infer_category_from_metadata(
            ...
            if self.metadata_extractor is not None and siglip_embedding is not None:
                logger.debug(
                    f"Calling MetadataExtractor.extract_all() with embedding shape: {siglip_embedding.shape}"
                )
                ...
                logger.debug(
                    f"MetadataExtractor returned: category={extracted.primary_category}, "
                ...
                metadata.mood_tags = (
                    extracted.mood_tags if extracted.mood_tags else ["balanced"]
                )
                ...
                logger.info(
                    f"Used MetadataExtractor: category={extracted.primary_category}, "
                    f"moods={extracted.mood_tags}, styles={extracted.style_tags}"
                )
            else:
            ...
            if quality_score:
                metadata.quality_tier = (
                    quality_score.quality_tier.capitalize()
                )  # premium -> Premium
                metadata.aesthetic_score = (
                    quality_score.raw_aesthetic
                )  # V2.5 raw score (1-10)
            else:
