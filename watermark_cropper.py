...
class WatermarkCropper:
    ...
    MIN_BOUNDARY_BRIGHTNESS_DELTA = 25
    ...
    def detect_reddit_watermark(self, img: Image.Image) -> Optional[int]:
        ...
        try:
            ...
            row_darkness = np.mean(
                np.all(bottom_array < self.DARK_THRESHOLD, axis=2),
                axis=1,
            )

            dark_rows = 0
            for dark_ratio in row_darkness[::-1]:
                if dark_ratio >= self.MIN_DARK_RATIO:
                    dark_rows += 1
                else:
                    break

            min_height, max_height = self.WATERMARK_HEIGHT_RANGE
            if dark_rows < min_height or dark_rows > min(max_height, max_check_height):
                return None

            strip_start = max_check_height - dark_rows
            if strip_start <= 0:
                return None

            boundary_row = bottom_array[strip_start - 1, :, :]
            strip = bottom_array[strip_start:, :, :]
            boundary_brightness = np.mean(boundary_row)
            strip_brightness = np.mean(strip)
            if (
                boundary_brightness - strip_brightness
                < self.MIN_BOUNDARY_BRIGHTNESS_DELTA
            ):
                return None

            logger.debug(
                f"Detected Reddit watermark: {dark_rows}px bar (boundary delta: {boundary_brightness - strip_brightness:.1f})"
            )
            return dark_rows
    ...
    def process_batch(self, filepaths: list[Path], sources: list[str]) -> dict:
        """
        ...
        """
        stats = {"total": len(filepaths), "reddit": 0, "cropped": 0, "failed": 0}
        ...
        logger.info(
            f"Watermark processing: {stats['cropped']}/{stats['reddit']} Reddit images cropped"
        )
        return stats
