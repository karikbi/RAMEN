...
class MLQualityScorer:
    ...
    def __init__(
        self, embedding_extractor=None, config: Optional[MLQualityConfig] = None
    ):
        """
    ...
    def _score_aesthetic_v25(self, image) -> Tuple[float, float]:
        ...
        pixel_values = self._aesthetic_processor(
    ...
    def _compute_siglip_similarity(
        self, image_features, positive_prompt: str, negative_prompt: str
        ...
        text_inputs = self._siglip_processor(
            text=[positive_prompt, negative_prompt], padding=True, return_tensors="pt"
        )
    ...
    def score(self, filepath) -> MLQualityScore:
        ...
        try:
            ...
            if aesthetic_loaded:
                result.aesthetic_score, result.raw_aesthetic = (
                    self._score_aesthetic_v25(img)
                )
            else:
            ...
            if siglip_loaded:
                ...
                image_inputs = self._siglip_processor(
                    images=img_siglip, return_tensors="pt"
                )
                ...
                with torch.no_grad():
                    image_features = self._siglip_model.get_image_features(
                        **image_inputs
                    )
            ...
            result.final_score = max(result.aesthetic_score, result.final_score)
