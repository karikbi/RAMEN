...
class ManifestManager:
    ...
    def _create_manifest_entry(
        ...
        entry = {
            ...
            # Quality (short keys)
            "qs": round(
                float(self._to_native(wallpaper.quality_scores.final_score)), 3
            ),  # quality_score
            ...
            "dark": self._to_native(
                meta.is_dark_mode_friendly
            ),  # is_dark_mode_friendly
            ...
            "emb": {
                ...
                "en2": self._encode_embedding(
                    embeddings.efficientnet_v2
                ),  # efficientnet_v2
    ...
    def save_collection(self, wallpapers: list[dict]) -> None:
        ...
        if self.config.compressed:
            output_path = self.config.manifests_dir / (
                self.config.collection_file + ".gz"
            )
        ...
        if self.r2_manager:
            try:
                ...
                r2_url = self.r2_manager.upload_manifest(
                    output_path, key=output_path.name
                )
    ...
    def _get_next_manifest_filename(
        self, date_str: str, test_mode: bool = False
    ) -> str:
        """
    ...
    def _save_date_manifest(
        self, date_str: str, wallpapers: list[dict], test_mode: bool = False
    ) -> Path:
        """
        ...
        logger.info(
            f"{mode_label} Saved manifest to temp (R2-only): {manifest_path} ({len(wallpapers)} wallpapers)"
        )
    ...
    def update_manifest(
        ...
        if self.r2_manager and manifest_path.exists():
            try:
                ...
                r2_key = (
                    f"test/{manifest_path.name}" if test_mode else manifest_path.name
                )
...
class SourceStatsManager:
    ...
    def update_stats(
        self, source: str, candidates: int, approved: int, subreddit: str = None
    ) -> None:
        """
