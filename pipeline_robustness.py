...
class StateManager:
    ...
    def save_state(self, state: PipelineState) -> None:
        ...
        logger.debug(
            f"State saved: stage={state.stage_completed}, processed={state.candidates_processed}"
        )
    ...
    def is_recent(self, max_age_hours: int = 24) -> bool:
        ...
        try:
            ...
            if is_recent:
                logger.info(
                    f"📋 Found recent state from {state.timestamp} (stage: {state.stage_completed})"
                )
            else:
                logger.info(
                    f"⏰ State too old ({age.total_seconds() / 3600:.1f}h), starting fresh"
                )
...
class EmbeddingProgressTracker:
    ...
    def load(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"📊 Loaded embedding progress for {len(self.progress)} wallpapers"
            )
        except Exception as e:
    ...
    def mark_complete(
        self, wallpaper_id: str, model: str, embedding_data: Any = None
    ) -> None:
    ...
    def check_exists_in_r2(self, r2_path: str, r2_client: Any) -> bool:
        ...
        try:
            r2_client.head_object(
                Bucket=os.getenv("R2_BUCKET_NAME", ""), Key=r2_path.lstrip("/")
            )
            return True
        except Exception:
            return False
...
class DataValidator:
    ...
    async def spot_check_r2_urls(
        self, urls: list[str], sample_size: int = 5
    ) -> tuple[bool, list[str]]:
...
class AtomicFileWriter:
    ...
    @contextmanager
    def atomic_write(
        self, target: Path, mode: str = "w", encoding: str = "utf-8"
    ) -> Iterator[IO]:
        """
...
class MemoryMonitor:
    ...
    def __init__(
        self, warn_threshold_mb: int = 3000, critical_threshold_mb: int = 3500
    ):
    ...
    def check_memory(self) -> tuple[int, str]:
        ...
        if usage_mb >= self.critical_threshold_mb:
            status = "critical"
            logger.error(
                f"🔴 CRITICAL: Memory usage at {usage_mb}MB (threshold: {self.critical_threshold_mb}MB)"
            )
        elif usage_mb >= self.warn_threshold_mb:
            status = "warning"
            logger.warning(
                f"🟡 WARNING: Memory usage at {usage_mb}MB (threshold: {self.warn_threshold_mb}MB)"
            )
        else:
            status = "ok"
...
class TimeoutManager:
    ...
    def __init__(self, max_runtime_minutes: int = 50, upload_reserve_minutes: int = 10):
        ...
        for api, available in creds.items():
            if not available:
                if api == "r2":
                    ...
                    logger.warning(
                        f"⚠️ {api.upper()} credentials missing (source will be skipped)"
                    )
...
class GracefulDegradation:
    ...
    def record_model_failure(self, model: str, error: str) -> None:
        ...
        logger.warning(
            f"⚠️ Model '{model}' failed, wallpapers will have incomplete embeddings"
        )

    def record_wallpaper_failure(
        self, wallpaper_id: str, stage: str, error: str
    ) -> None:
...
class BatchProcessor:
    ...
    def __init__(
        self, batch_size: int = 10, memory_monitor: Optional[MemoryMonitor] = None
    ):
    ...
    def generator_batches(self, items: list[Any]) -> Iterator[list[Any]]:
        """
