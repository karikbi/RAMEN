...
class FilteringPipeline:
    ...
    def __init__(
        ...
        if filter_config is None:
            ...
            filter_config = FilterConfig(
                ...
                max_file_size=int(central.get("filters.max_file_size_mb", 20))
                * 1024
                * 1024,
                ...
                hash_similarity_threshold=int(
                    central.get("filters.phash_threshold", 10)
                ),
            )
        ...
        self.hard_filters = HardFilters(
            self.filter_config, dedup_checker=self.dedup_checker
        )
        self.embedding_extractor = EmbeddingExtractor()
        self.ml_quality_scorer = MLQualityScorer(
            embedding_extractor=self.embedding_extractor, config=self.quality_config
        )
        ...
        if self.ml_quality_scorer._ensure_siglip_loaded():
            ...
            if dinov3_model is not None:
                ...
                logger.info(
                    "Initialized MetadataExtractor with SigLIP (gradient fallback for focal points)"
                )
    ...
    def _categorize_rejection(self, reason: str) -> None:
        ...
        if not reason_lower or reason_lower == "unknown":
            self.stats.rejected_other += 1
        elif "resolution" in reason_lower:
            ...
            "size" in reason_lower or "small" in reason_lower or "large" in reason_lower
        ):
            self.stats.rejected_file_size += 1
        elif (
            "duplicate" in reason_lower
            or "already fetched" in reason_lower
            or "already processed" in reason_lower
            or "exact content match" in reason_lower
            or "visual match" in reason_lower
        ):
            self.stats.rejected_duplicate += 1
        elif (
            "download" in reason_lower
            or "filepath" in reason_lower
            or "no valid filepath" in reason_lower
        ):
            self.stats.rejected_download_failed += 1
        else:
            self.stats.rejected_other += 1

    def process_candidate(
        self, candidate: CandidateWallpaper
    ) -> Optional[ApprovedWallpaper]:
        """
        ...
        if not filter_result.passed:
            ...
            self.hard_filters.reject_candidate(
                candidate.filepath, filter_result.reason or "unknown"
            )
        ...
        ml_score, siglip_embedding = self.ml_quality_scorer.score_for_source(
            candidate.filepath, source=candidate.source
        )
        ...
        logger.info(
            f"Approved {candidate.id} (score={ml_score.raw_aesthetic:.1f}/10) - extracting embeddings"
        )
        embeddings = EmbeddingSet()
        embeddings.siglip = siglip_embedding
        embeddings.mobilenet_v4 = self.embedding_extractor.extract_mobilenet_v4(
            candidate.filepath
        )
        embeddings.efficientnet_v2 = self.embedding_extractor.extract_efficientnet(
            candidate.filepath
        )
    ...
    def process_all(
        self, candidates: list[CandidateWallpaper], show_progress: bool = True
        ...
        iterator = (
            tqdm(candidates, desc="Pass 1: Hard Filtering")
            if show_progress
            else candidates
        )
        ...
        for candidate in iterator:
            try:
                ...
                filter_result = self.hard_filters.apply_all_filters(
                    candidate.filepath, candidate.id, source=candidate.source
                )
        ...
        logger.info(
            f"✅ Pass 1 complete: {len(passed_candidates)}/{len(candidates)} passed hard filters"
        )
        ...
        quality_passed: list[
            tuple[
                CandidateWallpaper, FilterResult, "MLQualityScore", Optional[np.ndarray]
            ]
        ] = []
        ...
        iterator2 = (
            tqdm(passed_candidates, desc="Pass 2: Quality Scoring")
            if show_progress
        ...
        for candidate, filter_result in iterator2:
            try:
                ...
                quality_passed.append(
                    (candidate, filter_result, ml_score, siglip_embedding)
                )
                logger.info(
                    f"✓ Approved {candidate.id} (score={ml_score.raw_aesthetic:.1f}/10)"
                )
        ...
        logger.info(
            f"✅ Pass 2 complete: {len(quality_passed)}/{len(passed_candidates)} passed quality check"
        )
        ...
        iterator3 = (
            tqdm(quality_passed, desc="Pass 3: Embeddings")
            if show_progress
            else quality_passed
        )
        ...
        for candidate, filter_result, ml_score, siglip_embedding in iterator3:
            try:
                ...
                other_embeddings = self.embedding_extractor.extract_optimized(
                    candidate.filepath, skip_siglip=True
                )
        ...
        logger.info(
            f"✅ Pass 3 complete: {len(approved)} wallpapers with full embeddings"
        )
...
def run_part2_pipeline(
    ...
    pipeline = FilteringPipeline(
        config=config, quality_config=quality_config, dedup_checker=dedup_checker
    )
