...
class EmbeddingSet:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            # V2 stack (primary)
            "mobilenet_v4": self.mobilenet_v4.tolist()
            if self.mobilenet_v4 is not None
            else None,
            "efficientnet_v2": self.efficientnet_v2.tolist()
            if self.efficientnet_v2 is not None
            else None,
...
class EmbeddingExtractor:
    ...
    def _ensure_image(
        self, input_data: Union[str, Path, Image.Image], size: tuple = None
    ) -> Image.Image:
        """
    ...
    def extract_siglip(
        ...
        try:
            ...
            with torch.no_grad():
                ...
                outputs = _extract_siglip_tensor(outputs)
    ...
    def calculate_siglip_quality_score(
        self, filepath
        ...
        try:
            ...
            with torch.no_grad():
                ...
                image_features = _extract_siglip_tensor(image_features)
    ...
    def _load_dinov3(self):
        ...
        if self._dinov3_model is None:
            try:
                ...
                if not token:
                    logger.warning(
                        "HF_TOKEN not set - DINOv3 may fail if terms not accepted locally"
                    )

                self._dinov3_processor = AutoImageProcessor.from_pretrained(
    ...
    def extract_optimized(
        self, filepath: Union[str, Path], skip_siglip: bool = False
    ) -> EmbeddingSet:
        """
    ...
    def extract_all_with_recovery(
        self, filepath, wallpaper_id: str, progress_tracker=None, memory_monitor=None
    ) -> EmbeddingSet:
        """
        ...
        logger.debug(
            f"Extracting embeddings for {wallpaper_id}, pending: {pending_models}"
        )
